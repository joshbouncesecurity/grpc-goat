...
import (
	...
	pb "grpc-goat/labs/grpc-004-arbitary-mtls/server/proto"
)
...
func (s *partnerServer) GetPartnerData(ctx context.Context, req *pb.PartnerDataRequest) (*pb.PartnerDataResponse, error) {
	...
	return &pb.PartnerDataResponse{
		...
		Partners: partners,
		Flag:     "GRPC_GOAT{arbitrary_mtls_bypasses_partner_auth}",
	}, nil
}
...
func main() {
	...
	log.Println("Partner API gRPC server starting on port 8004...")
	log.Println("WARNING: mTLS accepts ANY client certificate - no proper validation!")
	log.Println("Attackers can generate their own certificates to access partner data")
